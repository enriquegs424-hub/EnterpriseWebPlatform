    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-neutral-900 border-l-4 border-olive-500 pl-4">Monitor Global de Horas</h1>
                    <p className="text-neutral-500 mt-1 ml-5">Vista en tiempo real de todas las horas del equipo</p>
                </div>
                <button
                    onClick={exportToCSV}
            ...
            {/* Period Selector */}
            <div className="flex items-center space-x-2 bg-white rounded-xl p-1 border border-neutral-200 w-fit">
            ...
            {stats && !stats.error && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    >
                        ...
                        <p className="text-3xl font-bold text-neutral-900">{stats.avgHoursPerDay}h</p>
                        <p className="text-sm text-neutral-500 mt-1">Promedio diario</p>
                    </motion.div>
                    ...
                    >
                        ...
                        <p className="text-3xl font-bold text-neutral-900">{stats.topUsers.length}</p>
                    ...
                    >
                        ...
                        <p className="text-sm text-neutral-500 mt-1">Registros totales</p>
                    </motion.div>
                </div>
            )}
            ...
            {stats && !stats.error && (
                <div className="bg-white rounded-2xl border border-neutral-200 overflow-hidden shadow-sm">
                    <div className="bg-gradient-to-r from-neutral-50 to-neutral-100 px-6 py-4 border-b border-neutral-200 flex items-center justify-between">
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <p className="text-sm font-semibold text-neutral-500 mb-4 uppercase tracking-wider">Carga por Departamento</p>
                                ...
                                {Object.entries(
                                    ...
                                    <div key={dept} className="space-y-2">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-bold text-olive-700">{hours}h</span>
                                        </div>
                        ...
                        <div>
                            <p className="text-sm font-semibold text-neutral-500 mb-4 uppercase tracking-wider">Top 8 Distribución de Horas</p>
            ...
            <div className="bg-white rounded-2xl border border-neutral-200 p-6">
                <div className="flex items-center space-x-2 mb-4">
                    ...
                    <h3 className="font-bold text-neutral-900">Filtros de Búsqueda</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label className="block text-sm font-semibold text-neutral-700 mb-1">Usuario</label>
                        <select
                            value={filters.userId}
                            onChange={(e) => setFilters({ ...filters, userId: e.target.value })}
                            className="w-full p-2.5 border border-neutral-200 rounded-lg focus:ring-2 focus:ring-olive-500/20 outline-none text-sm"
                        >
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-neutral-700 mb-1">Desde</label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-neutral-700 mb-1">Hasta</label>
                        <input
                            type="date"
            ...
            {stats && !stats.error && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {stats.topUsers.map((user: any, idx: number) => (
                                <div key={user.userId} className="flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        <span className="flex items-center justify-center w-6 h-6 bg-neutral-100 rounded-full text-xs font-bold text-neutral-600">
                                            {idx + 1}
                                        </span>
                                        <div>
                    ...
                    {/* Top Projects */}
                    <div className="bg-white rounded-2xl border border-neutral-200 overflow-hidden shadow-sm">
                        <div className="bg-gradient-to-r from-info-50 to-info-100/50 px-6 py-4 border-b border-info-100">
                            <h3 className="font-bold text-neutral-900 flex items-center">
                            ...
                            {stats.topProjects.map((project: any, idx: number) => (
                                <div key={project.code} className="flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <p className="font-mono text-sm font-bold text-info-600">{project.code}</p>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-neutral-200 text-sm">
                        <thead className="bg-neutral-50">
                            <tr>
                                <th className="px-6 py-3 text-left font-semibold text-neutral-600">Fecha</th>
                                <th className="px-6 py-3 text-left font-semibold text-neutral-600">Usuario</th>
                                <th className="px-6 py-3 text-left font-semibold text-neutral-600">Proyecto</th>
                                <th className="px-6 py-3 text-left font-semibold text-neutral-600">Horas</th>
                                <th className="px-6 py-3 text-left font-semibold text-neutral-600">Notas</th>
                            </tr>
                        </thead>
                            ...
                            {entries.slice(0, 50).map((entry) => (
                                <tr key={entry.id} className="hover:bg-neutral-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div>
                                            <p className="font-semibold text-neutral-900">{entry.user.name}</p>
                                            <p className="text-xs text-neutral-500">{entry.user.department}</p>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div>
                                            <p className="font-mono text-olive-600 font-bold text-xs">{entry.project.code}</p>
                                            <p className="text-xs text-neutral-600">{entry.project.name}</p>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="font-bold text-olive-700">{entry.hours}h</span>
                                    </td>
                                    <td className="px-6 py-4 max-w-xs">
                                        <p className="text-neutral-600 truncate text-xs">{entry.notes || '—'}</p>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
