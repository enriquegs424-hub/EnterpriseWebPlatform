  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-neutral-900 dark:text-neutral-100 border-l-4 border-olive-500 pl-4">Gestión de Clientes</h1>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {clients.map((client) => (
          <motion.div
            ...
            className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm hover:shadow-md transition-all overflow-hidden group"
          >
            <div className="bg-gradient-to-r from-olive-50 to-olive-100/50 dark:from-olive-900/20 dark:to-olive-800/20 px-6 py-4 border-b border-olive-100 dark:border-olive-900/50">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="font-bold text-neutral-900 dark:text-neutral-100 text-lg">{client.name}</h3>
                  {client.company && (
                    <p className="text-sm text-olive-700 dark:text-olive-400 font-medium mt-0.5">{client.company}</p>
                  )}
                </div>
                <button
            ...
            <div className="p-6 space-y-3">
              ...
              <div className="pt-3 border-t border-neutral-100 dark:border-neutral-800 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-olive-600 dark:text-olive-400 hover:text-olive-700 dark:hover:text-olive-300 hover:bg-olive-50 dark:hover:bg-olive-900/20 p-2 rounded-lg transition-all"
                >
                  <Edit2 size={16} />
                </button>
              </div>
      ...
      {clients.length === 0 && !loading && (
        <div className="text-center py-12 bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800">
          ...
          <p className="text-neutral-500 dark:text-neutral-400 font-medium">No hay clientes registrados</p>
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-neutral-900 dark:text-neutral-100 flex items-center">
                  <Users className="w-5 h-5 mr-3 text-olive-600 dark:text-olive-500" />
                  {isCreating ? 'Nuevo Cliente' : 'Editar Cliente'}
                </h3>
                <button onClick={() => { setEditingClient(null); setIsCreating(false); }} className="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300 text-2xl">&times;</button>
              </div>

              <form onSubmit={isCreating ? handleCreate : handleUpdate} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">Nombre del Cliente *</label>
                    <input
                      value={editingClient.name}
                      onChange={e => setEditingClient({ ...editingClient, name: e.target.value })}
                      placeholder="Juan Pérez"
                      className="w-full p-2.5 border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100 rounded-lg focus:ring-2 focus:ring-olive-500/20 outline-none"
                      required
                    />
                  </div>

                  <div>
                    <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">Empresa</label>
                    <input
                ...
                <div>
                  <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">Dirección</label>
                  <textarea
                    value={editingClient.address || ''}
                    onChange={e => setEditingClient({ ...editingClient, address: e.target.value })}
                    placeholder="Calle Principal 123, Madrid, España"
                    rows={2}
                    className="w-full p-2.5 border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100 rounded-lg focus:ring-2 focus:ring-olive-500/20 outline-none resize-none"
                  />
                </div>
                ...
                {!isCreating && (
                  <div className="flex items-center space-x-3 pt-2">
                    ...
                    />
                    <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Cliente Activo</label>
                  </div>
                )}

                <div className="pt-6 flex space-x-3">
                  <button type="button" onClick={() => { setEditingClient(null); setIsCreating(false); }} className="flex-1 px-4 py-2 border border-neutral-200 dark:border-neutral-700 text-neutral-600 dark:text-neutral-300 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-800 font-medium transition-colors">Cancelar</button>
