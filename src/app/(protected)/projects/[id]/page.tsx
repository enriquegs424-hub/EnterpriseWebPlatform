...
export default function ProjectDetailsPage({ params }: { params: { id: string } }) {
    ...
    return (
        <div className="flex flex-col h-full bg-neutral-50 p-6 space-y-6 overflow-y-auto">
            {/* Header */}
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-neutral-200">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                    <div>
                        <div className="flex items-center space-x-3 mb-2">
                            <span className="bg-olive-100 text-olive-700 text-xs font-black px-2 py-1 rounded-md uppercase tracking-wider">
                                {project.code}
                            </span>
                            {project.client && (
                        ...
                        <h1 className="text-3xl font-black text-neutral-900 tracking-tight">{project.name}</h1>
                    </div>

                    <div className="flex items-center space-x-4 w-full md:w-auto">
                        <div className="flex flex-col items-end mr-4">
                            <span className="text-sm font-bold text-neutral-500 mb-1">Progreso</span>
                            <div className="flex items-center space-x-2">
                                <span className="text-2xl font-black text-olive-600">{project.progress}%</span>
                        ...
                        <button className="bg-neutral-900 text-white px-5 py-3 rounded-xl font-bold hover:bg-black transition-all shadow-lg">
                            Editar
                        </button>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-neutral-200">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-neutral-900 flex items-center">
                                <CheckSquare className="mr-2 text-olive-600" size={24} />
                                Tareas Recientes
                            </h2>
                            <Link href={`./${project.id}/tasks`} className="text-sm font-bold text-olive-600 hover:text-olive-700 flex items-center">
                        ...
                        <div className="space-y-3">
                            {project.tasks.length === 0 ? (
                                ...
                                project.tasks.slice(0, 5).map((task: any) => (
                                    <div key={task.id} className="flex items-center p-3 hover:bg-neutral-50 rounded-xl transition-colors border border-transparent hover:border-neutral-100 group">
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-neutral-200">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <Link href={`./${project.id}/documents`} className="text-sm font-bold text-olive-600 hover:text-olive-700 flex items-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {project.documents.length === 0 ? (
                                <div className="col-span-full text-center py-8 text-neutral-400 bg-neutral-50 rounded-2xl border border-dashed border-neutral-200">
                                    <FileText size={32} className="mx-auto mb-2 opacity-50" />
                                    <p>No hay documentos</p>
                                </div>
                            ) : (
                                project.documents.slice(0, 4).map((doc: any) => (
                                    <div key={doc.id} className="flex items-center p-4 bg-neutral-50 rounded-xl border border-neutral-100 hover:border-olive-200 transition-colors cursor-pointer">
                                        <div className="bg-white p-2 rounded-lg shadow-sm mr-3 text-olive-600">
                                            <FileText size={20} />
                                        </div>
                ...
                <div className="space-y-6">
                    {/* Quick Stats */}
                    <div className="bg-neutral-900 text-white p-6 rounded-3xl shadow-lg relative overflow-hidden">
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-neutral-200">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-xl font-bold text-neutral-900 flex items-center">
                                <Calendar className="mr-2 text-olive-600" size={20} />
                                Próximos Eventos
                            </h2>
                            <Link href={`./${project.id}/events`} className="text-xs font-bold text-olive-600 hover:text-olive-700">
                                Ver Calendario
                            </Link>
                        </div>
                        ...
                        <div className="space-y-4">
                            {project.events.length === 0 ? (
                                <p className="text-neutral-400 text-sm">No hay eventos programados</p>
                            ) : (
                                project.events.map((event: any) => (
                                    <div key={event.id} className="flex space-x-3 border-l-2 border-olive-500 pl-3 py-1">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-neutral-900 text-sm">{event.title}</h4>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-neutral-200">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-xl font-bold text-neutral-900 flex items-center">
                                <Users className="mr-2 text-olive-600" size={20} />
                                Equipo
                            </h2>
                            <span className="text-xs font-bold text-neutral-400">{team.length} miembros</span>
                        </div>
                        <div className="space-y-3">
                            {team.length === 0 ? (
                                <p className="text-neutral-400 text-sm">No hay miembros asignados</p>
                            ) : (
                                team.map((member: any) => (
                                    <div key={member.id} className="flex items-center space-x-3 p-2 hover:bg-neutral-50 rounded-xl transition-colors">
                                        <div className="w-10 h-10 rounded-full bg-olive-100 flex items-center justify-center text-olive-700 font-bold">
                                            {member.name.charAt(0)}
                                        </div>
                                        <div>
                                            <p className="font-bold text-neutral-800 text-sm">{member.name}</p>
